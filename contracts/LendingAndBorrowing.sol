...
// LendingHelper Library (merged into this file for Remix)
library LendingHelper {
    function indexOf(address user, address[] memory arr) internal pure returns (int256) {
    ...
    function isUserPresentIn(address user, address[] memory arr) internal pure returns (bool, int256) {
        int256 index = indexOf(user, arr);
        bool present = index >= 0 ? true : false;
...
contract LendingAndBorrowing is Ownable {
    ...
    function getRemainingCollateral(address user)public view returns (uint256){
       ...
       function getAmountInDollars(uint256 amount, address tokenAddress) public view returns (uint256){
          (uint256 dollarPerToken, uint256 priceDecimals) = oneTokenEqualsHowManyDollars(tokenAddress);
          
          // Get token decimals using low-level call
          uint256 tokenDecimals = 18; // Default to 18
          
          // Try to get actual decimals (some tokens like USDC have 6 decimals)
          ...
          // Convert to 18 decimal dollar amount
          // Formula: (amount * dollarPerToken * 10^18) / (10^priceDecimals * 10^tokenDecimals)
          // Simplified: (amount * dollarPerToken * 10^(18-tokenDecimals)) / 10^priceDecimals
          
          uint256 totalAmountInDollars;
          if (tokenDecimals <= 18) {
              totalAmountInDollars = (amount * dollarPerToken * (10 ** (18 - tokenDecimals))) / (10 ** priceDecimals);
          } else {
              // If token has more than 18 decimals (rare), scale down
              totalAmountInDollars = (amount * dollarPerToken) / (10 ** priceDecimals) / (10 ** (tokenDecimals - 18));
          }
