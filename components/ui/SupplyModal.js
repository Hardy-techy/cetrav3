...
export default function SupplyModal({ token, onClose, onSupply, onRefresh, txResult, txError }) {
  ...
  const setMaxAmount = () => {
    setAmount(token.walletBalance?.amount || '0');
  };
      ...
      <div className="bg-[#0B0E14] rounded-xl max-w-md w-full border border-white/[0.08] shadow-2xl relative overflow-hidden">
        ...
        <div className="p-6 border-b border-white/[0.08] relative z-10">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-white tracking-tight">Supply {token.name}</h2>
            <button
              onClick={onClose}
        ...
        <div className="p-6 relative z-10">
          {!txResult && !txError ? (
            <>
              ...
              <div className="mb-6">
                <div className="flex justify-between mb-2 px-1">
                  ...
                  <span className="text-sm text-gray-400">
                    Balance: <span className="text-white">{parseFloat(token.walletBalance?.amount || 0).toFixed(4)}</span>
                  </span>
                </div>
              ...
              <div className="bg-white/[0.03] rounded-xl p-4 mb-6 space-y-3 border border-white/[0.05]">
                <div className="flex justify-between text-sm items-center">
                  ...
                  <span className="text-emerald-400 font-semibold bg-emerald-400/10 px-2 py-0.5 rounded text-xs">
            ...
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Supply Successful!</h3>
              <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-4 mb-6">
                <p className="text-emerald-400 font-bold text-xl">{amount} {token.name}</p>
