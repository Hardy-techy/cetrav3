...
export default function TokenInfo({
  ...
  const tokenImages = {
    ...
    ADE: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSllrF9PNBf88kIx9USP5g73XDYjkMyRBaDig&usqp=CAU",
  };
  ...
  const addTokenToMetamask = async (token) => {
    ...
    const tokenImage = tokenImages[token.name];
  ...
  const addLAR = async (token) => {
    ...
    const tokenImage = tokenImages["LAR"];
